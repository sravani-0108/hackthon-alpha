...
class PostgreSqlPlatform extends knex_1.AbstractSqlPlatform {
    ...
    getIndexName(tableName, columns, type) {
        ...
        if (indexName.length > 63) {
            ...
            return `${indexName.substring(0, 55 - type.length)}_${core_1.Utils.hash(indexName, 5, hashAlgorithm)}_${type}`;
        }
        return indexName;
    }
    getDefaultPrimaryName(tableName, columns) {
        ...
        if (indexName.length > 63) {
            const hashAlgorithm = this.config.get('hashAlgorithm');
            return `${indexName.substring(0, 55 - 'primary'.length)}_${core_1.Utils.hash(indexName, 5, hashAlgorithm)}_primary`;
        }
        return indexName;
    }
    /**
