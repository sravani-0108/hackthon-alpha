...
class UnitOfWork {
    ...
    constructor(em) {
        ...
        this.identityMap = new IdentityMap_1.IdentityMap(this.platform.getDefaultSchemaName());
    ...
    initIdentifier(entity) {
        ...
        for (const pk of pks) {
            if (pk.kind === enums_1.ReferenceKind.SCALAR) {
                idents.push(new entity_1.EntityIdentifier(entity[pk.name]));
            }
            else if (entity[pk.name]) {
                this.initIdentifier(entity[pk.name]);
                idents.push((0, entity_1.helper)(entity[pk.name])?.__identifier);
            }
        }
        if (pks.length === 1) {
    ...
    async runHooks(type, changeSet, sync = false) {
        ...
        if (wrapped.__identifier) {
            const idents = Utils_1.Utils.asArray(wrapped.__identifier);
            let i = 0;
        ...
        for (const prop of props) {
            ...
            if (Utils_1.Utils.isCollection(ref)) {
                ref.getItems(false).some(item => {
                    ...
                    const isScheduledForInsert = cs?.type === ChangeSet_1.ChangeSetType.CREATE && !cs.persisted;
