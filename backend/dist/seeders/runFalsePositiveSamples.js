 ...
 */
async function seedFalsePositiveCritical() {
    ...
    try {
        ...
        for (const s of scenarios) {
           ...
           VALUES ($1,$2,$3,$4, NOW() - INTERVAL '4 years') RETURNING id`, [custId, s.account.num, s.account.type, s.account.balance]);
                accId = ins.rows[0].id;
            }
            const now = Date.now();
            for (const h of s.history) {
                const exists = await client.query(`SELECT 1 FROM transactions WHERE account_id=$1 AND amount=$2 AND transaction_date::date=$3::date LIMIT 1`, [accId, h.amount, new Date(now - h.daysAgo * 86400000)]);
             ...
             VALUES ($1,$2,$3,$4,$5,'India',$6,'Completed')`, [accId, h.amount, h.type, h.from, s.account.num, new Date(now - h.daysAgo * 86400000)]);
                }
            }
            ...
            if (existingAlert.rows.length && existingAlert.rows[0].transaction_id) {
                ...
                await client.query(`UPDATE transactions SET account_id=$1, amount=$2, transaction_type=$3, sender_account=$4, transaction_date=$5, country='India' WHERE id=$6`, [accId, s.trigger.amount, s.trigger.type, s.trigger.from, triggerDate, triggerTxnId]);
            }
            else {
           ...
           VALUES ($1,$2,$3,$4,$5,'India',$6,'Completed') RETURNING id`, [accId, s.trigger.amount, s.trigger.type, s.trigger.from, s.account.num, triggerDate]);
...
exports.default = seedFalsePositiveCritical;
//# sourceMappingURL=runFalsePositiveSamples.js.map