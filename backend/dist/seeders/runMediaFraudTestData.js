 ...
 */
async function seedMediaFraudTestData() {
    ...
    try {
        ...
        for (const s of scenarios) {
            ...
            if (existingCust.rows.length) {
                ...
                await client.query(`UPDATE customers SET name=$1, occupation=$2, country=$3, is_pep=$4, risk_score=$5, risk_category=$6, address=$7, pan=$8 WHERE id=$9`, [
                    s.customer.name,
           ...
           VALUES ($1,$2,$3,$4, NOW() - INTERVAL '3 years') RETURNING id`, [custId, s.account.num, s.account.type, s.account.balance]);
            ...
            if (existingAlert.rows.length && existingAlert.rows[0].transaction_id) {
                ...
                await client.query(`UPDATE transactions SET account_id=$1, amount=$2, transaction_type=$3, sender_account=$4, receiver_account=$5, transaction_date=$6 WHERE id=$7`, [accId, s.trigger.amount, s.trigger.type, s.trigger.from, s.trigger.to, triggerDate, triggerTxnId]);
            }
            else {
...
exports.default = seedMediaFraudTestData;
//# sourceMappingURL=runMediaFraudTestData.js.map