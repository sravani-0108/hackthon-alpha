...
class DecisionAgent {
    synthesize(customer, transaction, sanctions, pep, media) {
        const riskFactors = [];
        if (transaction.risk === 'High')
            riskFactors.push('High-value transaction anomaly');
        if (transaction.spikeMultiplier >= 5)
            riskFactors.push(`${transaction.spikeMultiplier}x spike vs average`);
        if (pep.pepMatch)
            riskFactors.push('PEP match');
        if (media.negativeNews)
            riskFactors.push(`${media.articleCount} adverse media articles`);
        ...
        if (sanctions.sanctionMatch)
            riskFactors.push('Sanctions list match');
        if (customer.customerRisk === 'High')
            riskFactors.push('High-risk customer profile');
        if (transaction.velocityPattern !== 'Normal')
            riskFactors.push(`Unusual velocity: ${transaction.velocityPattern}`);
        let overallRiskScore = 0;
        if (transaction.risk === 'High')
            overallRiskScore += 35;
        else if (transaction.risk === 'Medium')
            overallRiskScore += 20;
        if (pep.pepMatch)
            overallRiskScore += 25;
        if (media.negativeNews)
            overallRiskScore += Math.min(media.articleCount * 5, 20);
        if (sanctions.sanctionMatch)
            overallRiskScore += 40;
        if (customer.customerRisk === 'High')
            overallRiskScore += 15;
    ...
    decide(synthesis) {
        ...
        if (synthesis.sanctionMatch) {
            ...
            if (synthesis.pepMatch)
                reasoning.push('PEP involvement elevates severity');
            if (synthesis.negativeMedia)
                reasoning.push('Adverse media corroborates suspicion');
        }
        else if (synthesis.overallRiskScore >= 40 ||
            synthesis.pepMatch ||
            ...
            if (synthesis.pepMatch)
                reasoning.push('PEP match requires senior analyst review');
            if (synthesis.transactionRisk === 'High')
                reasoning.push('Transaction anomaly detected');
        }
        else {
            decision = 'CLEAR';
...
exports.default = new DecisionAgent();
