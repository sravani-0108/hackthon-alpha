...
import { buildMediaSearchPrompt, parseJsonFromLlm } from './utils';
...
function articlesFromGrounding(response: GenerateContentResponse): MediaAnalysisResult['articles'] {
  const chunks = response.candidates?.[0]?.groundingMetadata?.groundingChunks ?? [];
  return chunks
    .map((chunk) => chunk.web)
    .filter((web): web is NonNullable<typeof web> => Boolean(web?.title || web?.uri))
    .map((web) => ({
...
function buildResult(
  customerName: string,
  partial: Partial<MediaAnalysisResult> & Pick<MediaAnalysisResult, 'negativeNews' | 'articleCount' | 'articles' | 'summary'>
): MediaAnalysisResult {
  return {
    ...
    const articles =
      parsed.articles?.length
        ...
        : groundingArticles.length
          ? groundingArticles
          : (parsed as { article_urls?: string[] }).article_urls?.map((url) => ({
              title: url,
 ...
 */
export async function runGoogleSearchMedia(
  customerName: string,
  ...
  const prompt = buildMediaSearchPrompt({ ...context, customer_name: customerName });
  ...
  for (const model of SEARCH_MODELS) {
    for (let attempt = 1; attempt <= MAX_ATTEMPTS_PER_MODEL; attempt++) {
      try {
        ...
        if (parsed) {
          console.log(
            `[ADK] Google Search OK (${model}): negativeNews=${parsed.negativeNews}, articles=${parsed.articleCount}`
          );
          return parsed;
        }
        ...
        console.warn(
          `[ADK] Google Search (${model}) attempt ${attempt}/${MAX_ATTEMPTS_PER_MODEL}: ${lastError.slice(0, 120)}`
        );
