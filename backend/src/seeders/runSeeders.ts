...
async function seed(): Promise<void> {
  ...
  try {
    ...
    for (const country of HIGH_RISK_COUNTRIES) {
      await client.query(
        `INSERT INTO high_risk_countries (country_name, is_active, updated_at)
    ...
    const managerResult = await client.query(
      ...
      ['Rajesh Kumar', 'manager@bank.com', passwordHash, 'bank_manager']
    );
    ...
    for (const c of customers) {
      const r = await client.query(
        `INSERT INTO customers (customer_number, name, dob, address, pan, aadhaar, occupation, country, is_pep, risk_score, risk_category)
    ...
    const txns: Array<[number, number, string, string, string, string, Date]> = [
    ...
    await client.query(
      `INSERT INTO notifications (title, message, user_id, is_read) VALUES
       ('Welcome to AML System', 'Your account has been set up successfully.', $1, false),
       ('System Initialized', 'AML monitoring is now active.', $1, true)`,
      [managerId]
    );
