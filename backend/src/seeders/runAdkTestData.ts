 ...
 */
async function seedAdkTestData(): Promise<void> {
  ...
  try {
    ...
    for (const s of scenarios) {
      ...
      for (const h of s.history) {
        ...
        if (!exists.rows.length) {
          await client.query(
            ...
            [accId, h.amount, h.type, h.from, s.account.num, txDate]
          );
        }
      }
      ...
      if (existingAlert.rows.length && existingAlert.rows[0].transaction_id) {
        ...
        await client.query(
          `UPDATE transactions SET account_id=$1, amount=$2, transaction_type=$3, sender_account=$4, receiver_account=$5, transaction_date=$6, country='India' WHERE id=$7`,
    ...
    if (managerId && alertsAdded > 0) {
      await client.query(
        ...
        ['ADK Test Alerts Ready', `${alertsAdded} new ADK test alerts added. Open Alerts to run investigations.`, managerId]
      );
    }
...
export default seedAdkTestData;
