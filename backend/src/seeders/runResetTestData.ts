...
async function resetTestData(): Promise<void> {
  ...
  try {
    ...
    for (const s of scenarios) {
      ...
      const accIns = await client.query(
        `INSERT INTO accounts (customer_id, account_number, account_type, balance, opened_at)
         VALUES ($1,$2,$3,$4, NOW() - INTERVAL '2 years') RETURNING id`,
      ...
      for (const h of s.history) {
        await client.query(
          ...
          [accId, h.amount, h.type, h.from, 'to' in h ? (h.to ?? s.account.num) : s.account.num, day(h.daysAgo)]
        );
      }
    ...
    await client.query(
      ...
      ['5 Test Alerts Ready', 'Demo data reset. Five open alerts loaded for ADK workflow testing.', managerId]
    );
