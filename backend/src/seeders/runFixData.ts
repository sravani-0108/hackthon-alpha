 ...
 * Safe to re-run.
 */
async function fixData(): Promise<void> {
  ...
  try {
    ...
    const fixes: Array<{ alertCode: string; custNum: string; accountNum: string; amount: number; country: string; type: string; daysAgo: number }> = [
    ...
    for (const f of fixes) {
      ...
      if (!cust.rows.length || !acc.rows.length) continue;
      ...
      if (txnId) {
        await client.query(
          `UPDATE transactions SET account_id = $1, amount = $2, country = $3, transaction_type = $4, transaction_date = $5
           WHERE id = $6`,
          [acc.rows[0].id, f.amount, f.country, f.type, txnDate, txnId]
        );
      } else {
        const ins = await client.query(
          `INSERT INTO transactions (account_id, amount, transaction_type, sender_account, receiver_account, country, transaction_date, status)
           VALUES ($1,$2,$3,$4,'EXT-FIX',$5,$6,'Completed') RETURNING id`,
          [acc.rows[0].id, f.amount, f.type, f.accountNum, f.country, txnDate]
        );
        txnId = ins.rows[0].id;
      }

      await client.query(
        `UPDATE alerts SET customer_id = $1, transaction_id = $2 WHERE alert_code = $3`,
        [cust.rows[0].id, txnId, f.alertCode]
      );
    ...
    const baselines: Array<{ accountNum: string; amount: number; type: string; daysAgo: number }> = [
    ...
    for (const b of baselines) {
      ...
      const exists = await client.query(
        ...
        [acc.rows[0].id, b.amount, new Date(Date.now() - b.daysAgo * 86400000)]
      );
      if (exists.rows.length) continue;
      await client.query(
        `INSERT INTO transactions (account_id, amount, transaction_type, sender_account, receiver_account, country, transaction_date, status)
         VALUES ($1,$2,$3,'HIST','EXT-HIST','India',$4,'Completed')`,
        [acc.rows[0].id, b.amount, b.type, new Date(Date.now() - b.daysAgo * 86400000)]
      );
    }
    ...
    if (sneha.rows.length && snehaAcc.rows.length) {
      ...
      if (alt1012.rows.length) {
        ...
        if (alt1012.rows[0].transaction_id) {
          await client.query(
            `UPDATE transactions SET account_id = $1, amount = 2500000, country = 'India' WHERE id = $2`,
            [snehaAcc.rows[0].id, alt1012.rows[0].transaction_id]
          );
        }
      }
    }
