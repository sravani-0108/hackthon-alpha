import { Between, In, LessThan, MoreThanOrEqual, LessThanOrEqual } from 'typeorm';
import { AppDataSource } from '../config/database';
import { Transaction, TransactionCreationAttributes } from '../models/Transaction';
import { Account } from '../models/Account';
import { TransactionType } from '../types';

interface TransactionFilterParams {
  offset: number;
  limit: number | null;
  startDate?: string;
  endDate?: string;
  minAmount?: number;
  maxAmount?: number;
  transactionType?: TransactionType;
}

class TransactionRepository {
  private repo() {
    return AppDataSource.getRepository(Transaction);
  }

  private buildWhere(params: Omit<TransactionFilterParams, 'offset' | 'limit'>, accountIds?: number[]) {
    const where: Record<string, unknown> = {};

    if (accountIds) {
      where.account_id = In(accountIds);
    }

    if (params.startDate && params.endDate) {
      where.transaction_date = Between(new Date(params.startDate), new Date(params.endDate));
    } else if (params.startDate) {
      where.transaction_date = MoreThanOrEqual(new Date(params.startDate));
    } else if (params.endDate) {
      where.transaction_date = LessThanOrEqual(new Date(params.endDate));
    }

    if (params.minAmount !== undefined && params.maxAmount !== undefined) {
      where.amount = Between(params.minAmount, params.maxAmount);
    } else if (params.minAmount !== undefined) {
      where.amount = MoreThanOrEqual(params.minAmount);
    } else if (params.maxAmount !== undefined) {
      where.amount = LessThanOrEqual(params.maxAmount);
    }

    if (params.transactionType) {
      where.transaction_type = params.transactionType;
    }

    return where;
  }

  async findAll(params: TransactionFilterParams) {
    const { offset, limit, ...filters } = params;

    const [rows, count] = await this.repo().findAndCount({
      where: this.buildWhere(filters),
      relations: { account: { customer: true } },
      select: {
        account: { id: true, account_number: true, customer: { id: true, name: true, customer_number: true } },
      },
      order: { transaction_date: 'DESC' },
      ...(limit != null ? { skip: offset, take: limit } : {}),
    });

    return { count, rows };
  }

  async findByCustomerId(customerId: number, params: TransactionFilterParams) {
    const accounts = await AppDataSource.getRepository(Account).find({
      where: { customer_id: customerId },
      select: ['id'],
    });

    const ids = accounts.map((a) => a.id);
    if (ids.length === 0) return { count: 0, rows: [] as Transaction[] };

    const { offset, limit, ...filters } = params;

    const [rows, count] = await this.repo().findAndCount({
      where: this.buildWhere(filters, ids),
      order: { transaction_date: 'DESC' },
      ...(limit != null ? { skip: offset, take: limit } : {}),
    });

    return { count, rows };
  }

  async findById(id: number): Promise<Transaction | null> {
    return this.repo().findOne({
      where: { id },
      relations: ['account'],
    });
  }

  async create(transactionData: TransactionCreationAttributes): Promise<Transaction> {
    const transaction = this.repo().create(transactionData);
    return this.repo().save(transaction);
  }

  async countAll(): Promise<number> {
    return this.repo().count();
  }

  async countInTimeWindow(accountIds: number[], hours: number, beforeDate: Date): Promise<number> {
    const startDate = new Date(beforeDate);
    startDate.setHours(startDate.getHours() - hours);

    return this.repo().count({
      where: {
        account_id: In(accountIds),
        transaction_date: Between(startDate, beforeDate),
        status: 'Completed',
      },
    });
  }

  async countStructuringTransactions(
    accountIds: number[],
    maxAmount: number,
    hours: number,
    beforeDate: Date
  ): Promise<number> {
    const startDate = new Date(beforeDate);
    startDate.setHours(startDate.getHours() - hours);

    return this.repo().count({
      where: {
        account_id: In(accountIds),
        amount: LessThan(maxAmount),
        transaction_date: Between(startDate, beforeDate),
        status: 'Completed',
      },
    });
  }

  async getMonthlyVolume(accountIds: number[], year: number, month: number): Promise<number> {
    const startDate = new Date(year, month - 1, 1);
    const endDate = new Date(year, month, 0, 23, 59, 59);

    const result = await this.repo()
      .createQueryBuilder('t')
      .select('SUM(t.amount)', 'totalVolume')
      .where('t.account_id IN (:...accountIds)', { accountIds })
      .andWhere('t.transaction_date BETWEEN :startDate AND :endDate', { startDate, endDate })
      .andWhere('t.status = :status', { status: 'Completed' })
      .getRawOne<{ totalVolume: string | null }>();

    return parseFloat(result?.totalVolume || '0');
  }

  async getAverageMonthlyVolume(
    accountIds: number[],
    beforeDate: Date,
    excludeTransactionId?: number
  ): Promise<number> {
    if (!accountIds.length) return 0;

    const qb = this.repo()
      .createQueryBuilder('t')
      .select("DATE_TRUNC('month', t.transaction_date)", 'month')
      .addSelect('SUM(t.amount)', 'monthly_total')
      .where('t.account_id IN (:...accountIds)', { accountIds })
      .andWhere('t.status = :status', { status: 'Completed' })
      .andWhere('t.transaction_date < :beforeDate', { beforeDate });

    if (excludeTransactionId) {
      qb.andWhere('t.id != :excludeId', { excludeId: excludeTransactionId });
    }

    const monthlyTotals = await qb
      .groupBy("DATE_TRUNC('month', t.transaction_date)")
      .getRawMany<{ month: string; monthly_total: string }>();

    if (!monthlyTotals.length) return 0;

    const sum = monthlyTotals.reduce((acc, row) => acc + parseFloat(row.monthly_total || '0'), 0);
    return sum / monthlyTotals.length;
  }

  async getMonthlyVolumeExcludingTransaction(
    accountIds: number[],
    year: number,
    month: number,
    excludeTransactionId?: number
  ): Promise<number> {
    const startDate = new Date(year, month - 1, 1);
    const endDate = new Date(year, month, 0, 23, 59, 59);

    const qb = this.repo()
      .createQueryBuilder('t')
      .select('SUM(t.amount)', 'totalVolume')
      .where('t.account_id IN (:...accountIds)', { accountIds })
      .andWhere('t.transaction_date BETWEEN :startDate AND :endDate', { startDate, endDate })
      .andWhere('t.status = :status', { status: 'Completed' });

    if (excludeTransactionId) {
      qb.andWhere('t.id != :excludeId', { excludeId: excludeTransactionId });
    }

    const result = await qb.getRawOne<{ totalVolume: string | null }>();
    return parseFloat(result?.totalVolume || '0');
  }
}

export default new TransactionRepository();
